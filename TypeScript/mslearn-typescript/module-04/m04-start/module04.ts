...
let sortDescending: compareFuncType = function (a: number, b: number): number {
    if (a > b) {
        return -1;
    } else if (b > a) {
        return 1;
...
interface CompareFuncType {
    (a: number, b: number): number;
}

let sortAscending: CompareFuncType = (a: number, b: number): number => {
    if (a > b) {
        return 1;
    } else if (b > a) {
        return -1;
